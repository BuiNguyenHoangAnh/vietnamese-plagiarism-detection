...
def diff_lcs(source, input):
    ...
    return m/min(len(source.split()), len(input.split())), s
...
def diff_cs(source, input):
    ...
    pd.DataFrame(X_train_counts.toarray(), columns=count_vect.get_feature_names(), index = corpus)
